...
def nonPlanarFeatureExtraction(imagePath):
...
def cameracalibration(csvfilepath):
    pointList2D = []
    pointList3D = []
    ...
    return getcameraParameters(M,b,pointList2D,pointList3D)

def getcameraParameters(M,b,pointList2D,pointList3D):
    M = np.array(M)
    M.reshape(3,4)
    m1 = np.array(M[0])
    m2 = np.array(M[1])
    ...
    alphau = pow(float(pow(pDet, 2)*np.dot(a2,a2) - pow(s,2) - pow(v0, 2)),1/2)
    alphau = round(alphau,2)

    K = np.array([[alphau, s, u0],[0,alphav,v0],[0,0,1]])
    e = np.sign(b[2])

    T = e * pDet * np.dot(np.linalg.inv(K),b)
...
def implementRansac(k_max, n, count , totalset ,l,pointList):
...
nonPlanarFeatureExtraction(imagePath)


#Camera Calibration
print("Non planar camera calibration")
imgPoints = input("Enter Image point location")
objPoints = input("Enter Object point location")
#imgPoints = 'D:/CS-512/Assignments/AS4/Non-planar/ncc-imagePt.txt'
#objPoints = "D:/CS-512/Assignments/AS4/Non-planar/ncc-worldPt.txt"
...
mse,K, T, R = cameracalibration(csvfilepath)
...
noiseImgPoints = input("Enter Image point location")
noiseObjPoints = input("Enter Object point location")
# noiseImgPoints = 'D:/CS-512/Assignments/AS4/Non-planar/Noise1/ncc-noise-0-imagePt.txt'
# noiseObjPoints = "D:/CS-512/Assignments/AS4/Non-planar/Noise1/ncc-worldPt.txt"
...
all_S=[]
all_len=[]
M, b1 = implementRansac(max_k,n,count,all_S,all_len,list(content))
mseN,KN, TN, RN = getcameraParameters(M, b1,noise2DPointList,noise3DPointList)
print("Estimated Parameters:")
